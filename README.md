...
- **급식 패턴 분석 (datasci)** — 한 학기치 급식 데이터를 수집해 알레르겐 출현 빈도, 동시 출현
  상관관계, 요일별 노출, 주차별 추세를 분석하고 선형회귀로 다음 주 위험 알레르겐을 예측
  (`analyze.py`로 실행, 분석 탭에서 결과 확인 + PPT용 그래프 PNG 저장)
- **📷 식품 스캔 (물리 AI)** — 웹캠으로 음식을 비추면 Teachable Machine 이미지 분류 모델이
  실시간으로 식품군을 인식해 알레르겐을 추정하고, 내 알레르기 프로필과 대조해 안전/주의/위험을 표시
